  ...
  'number': (param) => {
    ...
    if (param.range) {
      if (param.range.min !== null) {
        json.minimum = param.range.min;
      }
      if (param.range.max !== null) {
        json.maximum = param.range.max;
      }
    }
    if (param.options && param.options.values) {
      json.enum = param.options.values;
...
const convertParamToJSONSchema = (param) => {
  let convert = APIToJSONSchemaMapping[param.type];
...
const generateJSONSchema = (definitions, origin = 'localhost', opts = {}) => {
  opts = opts || {};
  const includeFilenames = !!opts.includeFilenames;
  const schema = Object.keys(definitions)
    .filter(key => {
      let def = definitions[key];
      return def.format.language !== 'static' && !def.private;
    })
    .map(key => {
      let def = definitions[key];
      let method = '';
      if (key.match(/^(.*?)\#(.*)$/gi)) {
        key.replace(/^(.*?)\#(.*)$/gi, ($0, $1, $2) => {
          key = $1;
          method = $2;
        });
      }
      ...
      const schema = {
        name: name + (!method ? `` : `_${method.toLowerCase()}`),
        description: def.description,
        route: route,
        url: `${origin}${route}`,
        method: method || 'POST',
        parameters: convertParametersArrayToJSONSchema(def.params),
        endpoint_name: def.name,
      };
      if (def.returns && def.returns.type !== 'object.http') {
        schema.returns = convertParamToJSONSchema(def.returns);
      }
      if (includeFilenames) {
        schema.filename = def.pathname;
      }
      return schema;
    });
  return schema;
...
const generateOpenAPISpec = (definitions, plugin, server, origin, identifier) => {
  ...
  const paths = Object.keys(definitions)
    ...
    .reduce((paths, key) => {
      ...
      let operationId = `${opIdentifier}${parts.length ? ('.' + parts.join('.')) : ''}_${method.toLowerCase()}`;
      ...
      if (def.params.length > 0) {
        const schema = {...convertParametersArrayToJSONSchema(def.params)};
        if (method === 'GET' || method === 'DELETE') {
          pathData.parameters = def.params.map(param => {
            let p = {
              in: 'query',
              name: param.name,
              schema: schema.properties[param.name]
            };
            if (param.hasOwnProperty('defaultValue')) {
              p.required = true;
            }
      ...
      paths[route][`${method.toLowerCase()}`] = pathData;
      return paths;
    }, {});
...
const wellKnowns = {
  ...
  validatePlugin: (rawPlugin, origin) => {
    ...
    plugin.version = rawPlugin.version || (process.env.NODE_ENV || 'development');
  ...
  handlers: {
    ...
    '.well-known/schema.json': (definitions, plugin, server, origin, identifier) => {
      ...
      const schema = generateJSONSchema(definitions, origin);
