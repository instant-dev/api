...
class Gateway extends EventEmitter {
  ...
  constructor (cfg) {
    ...
    this.developmentMode = ('developmentMode' in cfg)
      ? !!cfg.developmentMode
      : false;
  ...
  __validateParameters__ (parsed, definition) {
    ...
    let paramsList = definition.params.map(param => {
      ...
      if (value === undefined) {
        ...
        try {
          value = types.sanitize(param.type, value, param.range, param.size);
        } catch (e) {
          errors[param.name] = {
            message: e.message,
            invalid: true
  ...
  async __requestHandler__ (req, res, definition, data, functionArgs, headers) {
    ...
    await this.execute(definition, req.method, functionArgs, data, headers, (err, value, headers, executionUuid) => {
      ...
      if (err !== null) {
        ...
        try {
          value = types.sanitize(definition.returns.type, value, definition.returns.range, definition.returns.size);
        } catch (e) {
          let details = {
            returns: {
              message: e.message,
  ...
  findDefinition (definitions, name, method) {
  ...
  resolve (req, res, buffer, callback) {
    ...
    let definition;
    try {
      definition = this.findDefinition(this.definitions, pathname, req.method);
    } catch (e) {
      if (e.notImplemented) {
        e.statusCode = 501;
      } else {
  ...
  __stream__ (streams, executionUuid, logId, eventName, value) {
    ...
    if (!stream) {
      ...
      if (
        ...
        try {
          value = types.sanitize(stream.type, value, stream.range, stream.size);
        } catch (e) {
          let details = {};
          details[eventName] = {
            message: e.message,
  ...
  async execute (definition, method, functionArgs, data, headers, callback) {
    ...
    if (definition.format.language === 'static') {
        ...
        if (!importModule) {
          try {
            ...
            if (this.preloadFiles[pathname]) {
              ...
              importModule = await import(rpath);
      ...
      /**
       * DEV MODE ONLY BEGIN
       * overwrite console.log for streaming to console
       */
      ...
      d.on('error', e => {
        ...
        e.thrown = true;
        this.developmentMode && (global.console = console);
        return callbackWrapper(e, null, headers, executionUuid);
      });
      d.run(async () => {
        try {
