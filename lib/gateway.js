...
class Gateway extends EventEmitter {
  ...
  __modelContextProtocolResponse__ (req, res, status, json) {
  ...
  __validateModelContextProtocolRequest__ (mcpRequest) {
    if (typeof mcpRequest !== 'object' || Array.isArray(mcpRequest)) {
    ...
    if (typeof mcpRequest.params !== 'object' || Array.isArray(mcpRequest.params)) {
      throw new Error(`Invalid request: invalid "params", must be an object`);
    }
  }
    ...
    req.on('end', () => {
      ...
      this.resolve(req, res, buffer, async (err, definition, data, buffer, proxyParameters) => {
        ...
        if (definition.hasOwnProperty('mcpEndpoint')) {
          ...
          const mcpResponses = await Promise.all(mcpRequests.map(async (mcpRequest) => {
            return (async () => {
              ...
              if (!mcpEndpoint[mcpRequest.method]) {
                ...
                try {
                  return await mcpEndpoint[mcpRequest.method](definitions, mcpRequest, req, this.findDefinition.bind(this));
                } catch (e) {
                  return {
                    jsonrpc: '2.0',
                    id: mcpRequest.id ?? null,
          ...
          return this.__modelContextProtocolResponse__(req, res, 200, mcpResponses);
        }
