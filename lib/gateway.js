...
class Gateway extends EventEmitter {
  ...
  __badRequestError__ (req, res, message, status, headers, stack) {
    headers = headers || {};
    headers = this.__createHeaders__(req, headers);
    headers['content-type'] = 'application/json';
      ...
      this.resolve(req, res, buffer, (err, definition, data, buffer, proxyParameters) => {
        ...
        if (err) {
          if (err.accessSourceError) {
            ...
            return this.__badRequestError__(req, res, err.message, err.statusCode);
      ...
      if (err !== null) {
        if (!(err instanceof Error)) {
          ...
          if (err.badRequest || err.statusCode === 400) {
            return this.__badRequestError__(req, res, message, err.statusCode, headers, err.stack);
          } else if (err.unauthorized || err.statusCode === 401) {
          ...
          if (err.badRequest || err.statusCode === 400) {
            return this.__badRequestError__(req, res, err.message, err.statusCode, headers, err.stack);
          } else if (err.unauthorized || err.statusCode === 401) {
