...
class Gateway extends EventEmitter {
  ...
  __chargeConflictError__ (req, res, message, status, headers, stack) {
    headers = headers || {};
    headers = this.__createHeaders__(req, headers);
    headers['content-type'] = 'application/json';
    if ((process.env.NODE_ENV || 'development') === 'production') {
  ...
  __conflictError__ (req, res, message, details, status, headers, stack) {
    headers = headers || {};
    headers = this.__createHeaders__(req, headers);
    headers['content-type'] = 'application/json';
    if ((process.env.NODE_ENV || 'development') === 'production') {
  ...
  /**
   * Builds an RFC 6266 Content-Disposition header value for a filename,
   * with an ASCII-safe `filename` and a UTF-8 `filename*` for non-ASCII names
   * @param {string} filename
   * @returns {string}
   */
  __formatContentDisposition__ (filename) {
    filename = String(filename)
      .split(/[\/\\]/).pop() // strip any directory components
      .replace(/[\x00-\x1f\x7f]/g, ''); // strip control characters
    // eslint-disable-next-line no-control-regex
    const isAscii = /^[\x20-\x7e]*$/.test(filename);
    const asciiName = (isAscii ? filename : filename.replace(/[^\x20-\x7e]/g, '_'))
      .replace(/["\\]/g, '\\$&');
    let value = `attachment; filename="${asciiName}"`;
    if (!isAscii) {
      value += `; filename*=UTF-8''${encodeURIComponent(filename)}`;
    }
    return value;
  }
  ...
  __complete__ (req, res, body, headers, statusCode) {
    headers = this.__createHeaders__(req, headers);
    if (Buffer.isBuffer(body) && body.filename) {
      // `filename` is shorthand: infer Content-Type from the extension and
      // build a Content-Disposition, unless either was set explicitly
      const inferredType = mime.getType(String(body.filename));
      if (!body.contentType && inferredType) {
        body.contentType = inferredType;
      }
      body.contentDisposition = body.contentDisposition || this.__formatContentDisposition__(body.filename);
    }
    ...
    if (Buffer.isBuffer(body) && body.contentDisposition && !headers['content-disposition']) {
      headers['content-disposition'] = body.contentDisposition;
      // Content-Disposition is not a CORS-safelisted response header,
      // so expose it for browser clients reading the filename
  ...
  __validateModelContextProtocolRequest__ (mcpRequest) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(Object.prototype, mcpRequest.method) ||
      !/^[a-zA-Z0-9_\-\.\/]+$/.test(mcpRequest.method)
    ) {
      throw new Error(`Invalid request: invalid "method", must be a non-empty string`);
    }
    if (
  ...
  __validateParameters__ (parsedParams, definition) {
    ...
    let paramsList = definition.params.map(param => {
      ...
      if (value === undefined) {
        ...
        try {
          value = types.sanitize(
            param.type, value, param.defaultValue === null, param.range, param.size,
            param.members ||
              (param.alternateSchemas || []).concat(param.schema ? [param.schema] : []),
            param.alternateTypes || [],
            [param.name]
    ...
    req.on('end', () => {
      ...
      this.resolve(req, res, buffer, async (err, definition, data, buffer, proxyParameters) => {
        ...
        if (err) {
          if (err.accessSourceError) {
          ...
          } else if (err.chargeConflictError) {
            return this.__chargeConflictError__(req, res, err.message);
          } else if (err.rateLimitError || err.statusCode === 429) {
            ...
            return this.__conflictError__(req, res, err.message, err.details, err.statusCode);
          }
      ...
      if (err !== null) {
        if (!(err instanceof Error)) {
          ...
          if (err.badRequest || err.statusCode === 400) {
            ...
            return this.__conflictError__(req, res, message, err.details, err.statusCode, headers, err.stack);
          } else if (err.rateLimitError || err.statusCode === 429) {
            return this.__rateLimitError__(req, res, message, err.rate && err.rate.count, err.rate && err.rate.period, headers);
          } else {
            this._errorHandler(err);
        ...
        try {
          value = types.sanitize(
            definition.returns.type, value, definition.returns.defaultValue === null, definition.returns.range, definition.returns.size,
            definition.returns.members ||
              (definition.returns.alternateSchemas || []).concat(definition.returns.schema ? [definition.returns.schema] : []),
            definition.returns.alternateTypes || [],
            [definition.returns.name || '$']
  ...
  createContext (req, definition, params, data, buffer) {
    ...
    context.stream = this.__stream__.bind(this, definition, context.uuid, null);
    context.charge = (amount) => { context.stream('@charge', amount); };
  ...
  __streamError__ (executionUuid, logId, eventName, message) {
    message = message || [
  ...
  __stream__ (definition, executionUuid, logId, eventName, value) {
    value = value === void 0
      ? null
      : value;
    let stream = (definition.streams || []).find(param => param.name === eventName);
    if (!stream) {
      if (eventName === '@charge') {
        const maxCharge = parseInt(definition.charge) || 0;
        if (maxCharge <= 0) {
          this.__streamError__(
            executionUuid,
            logId,
            eventName,
            [
              'The @charge stream is not enabled for this function.',
              'You must set a preauthorized charge amount using the @charge directive.'
            ].join(' ')
          );
          return false;
        } else {
          let streamType = 'integer';
          const range = { min: 0, max: maxCharge };
          let nullable = false;
          let details = {};
          let streamMismatch = {};
          if (
            !types.validate(
              streamType, value, nullable,
              [],
              [],
              void 0,
              range,
            ...
            try {
              value = types.sanitize(streamType, value, nullable, range, void 0);
            } catch (e) {
              let details = {};
              details[eventName] = {
                message: e.message,
      ...
      if (
        ...
        try {
          value = types.sanitize(
            stream.type, value, stream.defaultValue === null, stream.range, stream.size,
            stream.members ||
              (stream.alternateSchemas || []).concat(stream.schema ? [stream.schema] : []),
            stream.alternateTypes || [],
            [stream.name || '$']
  ...
  async execute (definition, method, functionArgs, data, headers, callback) {
    ...
    let callbackWrapper = (err, result, headers, executionUuid) => {
      if (!complete) {
        complete = true;
        // Clear the execution timeout immediately: otherwise the pending
        // Timeout retains this closure (and the entire request context)
        // for the full defaultTimeout after the request has completed
        if (timeout !== null) {
          clearTimeout(timeout);
          delete this._timeouts[timeout];
          timeout = null;
