...
class Gateway extends EventEmitter {
  ...
  createContext (req, definition, params, data, buffer) {
    ...
    const platformKeys = data?.platform_keys || {};
    context.platform = {
      getKey: (scope, key) => {
        if (!platformKeys.global?.enabled) {
          throw new Error(
            `This function does not have access to platform keys.\n` +
            `Platform keys are restricted to administrator accounts.`
          );
        }
        if (!platformKeys[scope]?.enabled) {
          throw new Error(
            `You do not currently have access to "${scope}" platform keys.\n` +
            `Try running this function again from within "${scope}" to get it to work.`
          );
        }
        const entries = platformKeys[scope] || {};
        const value = key in entries ? entries[key] : null;
        return value;
      }
    };
    return context;
