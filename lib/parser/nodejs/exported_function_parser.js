...
class ExportedFunctionParser {
      ...
      if (!commentParams.length) {
        ...
        if (param.hasOwnProperty('defaultValue')) {
          if (
            ...
            try {
              types.sanitize(
                defParam.type, param.defaultValue, param.defaultValue === null, defParam.range, defParam.size,
                defParam.members ||
                  (defParam.alternateSchemas || []).concat(defParam.schema ? [defParam.schema] : []),
                defParam.alternateTypes || [],
                [defParam.name]
  ...
  parse (name, buffer, pathname) {
    ...
    return Object.keys(endpoints).map(method => {
      ...
      let activity = commentDefinition.activity;
      let isPrivate = commentDefinition.private || false;
      let origins = commentDefinition.origins || null;
      let background = commentDefinition.background || null;
      let charge = typeof commentDefinition.charge === 'number'
        ? commentDefinition.charge
        : 0;
