  ...
  parseExportStatements (fileString) {
    ...
    let exportNamedStatements = body
      ...
      .forEach(item => {
        ...
        const allowedNames = ['get', 'post', 'put', 'del'];
    ...
    if (sourceType === 'script') {
      ...
      if (exportStatements) {
        if (exportStatements.default) {
          let statementObject = exportStatements.default;
          comment = this.parseCommentFromStatement(statementObject);
          functionExpression = this.parseFunctionExpressionFromExportStatement(statementObject);
        }
      }
    }
