...
class ExportedFunctionParser {
  ...
  parseExportStatements (fileString) {
    ...
    let exportDefaultStatements = body.filter(item => {
    ...
    return {
      default: exportDefaultStatements[0] || null
    };
  ...
  parse (name, fileString, pathname, buffer) {
    ...
    if (sourceType === 'script') {
      ...
      if (exportStatements) {
        if (exportStatements.default) {
          let statement = exportStatements.default;
          comment = this.parseCommentFromStatement(statement);
          functionExpression = this.parseFunctionExpressionFromExportStatement(statement);
        }
      }
    }
