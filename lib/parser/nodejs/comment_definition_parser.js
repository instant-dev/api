...
class CommentDefinitionParser {
  ...
  getParameter (values, textSchema = [], depth = 0, returnRawType = false) {
    ...
    value = value.slice(match.length).trim();
    if (type.startsWith('?')) {
      type = type.slice(1);
    ...
    if (typeRange) {
      if (typeRange.includes(',')) {
        range = typeRange.split(',').map(v => parseFloat(v));
      } else {
        ...
        if (
          parseInt(minSize) !== minSize ||
          parseInt(maxSize) !== maxSize
        ) {
          ...
          throw new Error(`Size {${typeRange}} invalid: max must be greater than 0`);
        }
      }
    }
