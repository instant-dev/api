...
const ACTIVITY_VARIABLE_NAME = /^[$_\p{ID_Start}][$\u200C\u200D_\p{ID_Continue}]*/u;
...
class CommentDefinitionParser {
  ...
  reduceLines (semanticsList, line) {
    ...
    if (line[0] === '@') {
      ...
      if (
        ...
        (field === 'param' || field === 'returns' || field === 'stream') &&
        previous &&
        ...
        // If we have a top level array we treat it like the first entry
        if (names[0].endsWith('[]')) {
          schemaParam = schemaParam.schema[0];
          firstName = '';
        }
        if (firstName === schemaParam.name) {
          ...
          while (names.length) {
            ...
            if (schemaParam) {
              ...
              if (isArray) {
                if (schemaParam.type !== 'array') {
                  throw new Error(`Parameter "${identifiers.join('.')}" is not an array, please use "${identifiers.join('.').slice(0, -2)}.${names.join('.')}" to reference schema members`);
                }
                let arrParam = curSchema[0];
                if (arrParam) {
                  curSchema = arrParam.schema || [];
                }
              } else if (schemaParam.type === 'array' && names.length) {
                throw new Error(`Parameter "${identifiers.join('.')}" is an array, please use "${identifiers.join('.')}[].${names.join('.')}" to reference schema members`);
              }
              if (!names.length) {
                throw new Error(`Already defined "${identifiers.join('.')}"`);
              }
  ...
  getOrigin (values) {
    ...
    let origins = [];
    if (value.startsWith('=')) {
      let envVar = value.slice('='.length);
      if (!envVar.startsWith('process.env.')) {
        throw new Error(
          `Invalid origin: "${value}". ` +
          `When assigning an environment variable, must start with "=process.env."`
        );
      }
      envVar = envVar.slice('process.env.'.length);
      if (!process.env[envVar]) {
        throw new Error(
          `Invalid origin: "${value}". ` +
          `No environment variable matching "${envVar}" found`
        );
      } else {
        from = ` (from "${value}")`;
  ...
  validateActivity (definition) {
    if (definition.activity === null) {
      return;
    }

    let activity = definition.activity;
    let paramNames = definition.params.map(param => param.name);
    for (let i = 0; i < activity.length; i++) {
      if (activity[i] !== '$') {
        continue;
      }

      let match = activity.slice(i + 1).match(ACTIVITY_VARIABLE_NAME);
      if (!match || !validateParameterName(match[0])) {
        throw new Error(
          `Comment Definition Error ("activity"): Invalid activity variable at character ${i}`
        );
      }

      let variableName = match[0];
      if (paramNames.indexOf(variableName) === -1) {
        throw new Error(
          `Comment Definition Error ("activity"): ` +
          `Activity variable "$${variableName}" does not match a defined parameter`
        );
      }
  ...
  getCharge (values) {
    let value = values.join(' ').trim();
    if (!value.length) {
      throw new Error('Invalid charge: missing value');
    }
    if (!value.match(/^\d+$/)) {
      throw new Error(`Invalid charge: "${value}". Must be a non-negative integer`);
    }
  ...
  getParameter (values, textSchema = [], depth = 0, returnRawType = false) {
    ...
    let isOptional = typedef[1].startsWith('?');
    let typeListStr = isOptional ? typedef[1].slice(1) : typedef[1];
    let typeList = [''];
    let complements = {'{': '}', '[': ']', '(': ')', '<': '>'};
    let braces = [];
    ...
    if (typeRange) {
      if (typeRange.includes(',')) {
        // An empty entry ({1,} or {,100}) means unbounded; "0" must be preserved
        range = typeRange.split(',').map(v => v.trim() === '' ? null : parseFloat(v));
      } else {
    ...
    if (schema || textSchema.length) {
      ...
      if (schema) {
        ...
        if (textSchema.length) {
          if (param.type === 'array') {
            if (!param.schema[0] || !SUPPORTS_SCHEMA[param.schema[0].type]) {
              throw new Error(`Can not provide a text schema for "${param.name}" (${param.type})`);
            }
            let schemas = this.parseSchemas(textSchema, depth, param.schema[0].type === 'object');
            param.schema[0].schema = schemas.shift();
            if (schemas.length) {
              param.alternateSchemas = schemas;
            }
          } else {
            throw new Error(`Can not provide text schema and JSDoc schema`);
          }
        }
      } else {
  ...
  createDefinition (definition, data) {
    ...
    try {
      ...
      if (field === 'description') {
        ...
        definition.activity = values.join('\n');
      } else if (field === 'param') {
  ...
  parse (name, commentString) {
    ...
    this.validateActivity(definition);
    return definition;
