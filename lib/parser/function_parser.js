...
class FunctionParser {
  ...
  getParser (ext) {
    ...
    if (!lang) {
      throw new Error(`No function parser available for extension "${ext}"`);
    } else if (!this._parsers[lang]) {
      throw new Error(`No function parser available for language "${lang}"`);
    }
  ...
  parseDefinition (pathname, buffer, functionsPath, isStatic, isPreload) {
    ...
    if (isStatic) {
      ...
      try {
        ...
        throw new Error(`Invalid endpoint file: (${pathname})\n${e.message}`);
      }
      let suffix;
      if (name === '__main__' || name === 'index') {
        name = names.join('/');
      } else if (name === '__notfound__' || name === '404') {
        name = names.join('/');
        suffix = 'notfound';
      } else {
      ...
      if (name && !name.match(/^([A-Z][A-Z0-9\_\-]*\/)*[A-Z][A-Z0-9\_\-]*$/i)) {
        throw new Error(
          `Invalid endpoint file: "${name}" (${pathname})\n` +
          `File and directory names must be alphanumeric (or -, _) and start with a letter`
        );
      }
      ...
      try {
        definitions = parser.parse(name, buffer, pathname);
        for (const definition of definitions) {
          definition.pathname = pathname;
        }
      } catch (e) {
        e.message = `Endpoint definition error (${pathname})\n${e.message}`;
        throw e;
      }
...
FunctionParser.definitionFields = {
  ...
  'activity': activity => activity === null || typeof activity === 'string',
  'metadata': metadata => typeof metadata === 'object' && metadata !== null,
  'private': isPrivate => isPrivate === void 0 || isPrivate === true || isPrivate === false,
  'origins': origins => {
  ...
  'params': (params, isArraySchema, ignoreName = false) => {
    return Array.isArray(params) &&
      ...
      params.filter(param => {
        return param &&
          typeof param == 'object' &&
          'name' in param &&
          (param.name || ignoreName) && // only require name if it's not an array schema
          ...
          (
            'schema' in param
              ? (
                ...
                FunctionParser.definitionFields['params'](param.schema, param.type === 'array', true)
              )
              : true
          ) &&
          (
          ...
          (
            'alternateTypes' in param
            ? (
                Array.isArray(param.alternateTypes) &&
                FunctionParser.definitionFields['params'](param.alternateTypes, false, true)
              )
            : true
          ) &&
