  ...
  'object.keyql.limit': (v, range) => {
    ...
    if (range) {
      if (v.count < range.min) {
        ...
        throw new Error(`count must be less than or equal to ${range[1]}`);
      }
    }
    return v;
  },
...
module.exports = {
  ...
  sanitize: (type, v, nullable = false, range = null, size = null) => {
    if (nullable && v === null) {
      return v;
    } else {
      return type in _sanitizations ? _sanitizations[type](v, range, size) : v
    }
  },
